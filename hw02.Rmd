...
birthday = function(n,k,i=1000){
  ...
  for(sim in 1:i){
    ...
    count_bdays = data.frame(table(bdays)) #data frame of birthday counts per birthday
    k_shared_bdays = subset(count_bdays, Freq >= k)$Freq #cases where at least k people share a birthday
    
    #if there is at least one value in k_shared_bdays, then the number of sims_satisfied is incremented by one
    if(length(k_shared_bdays) > 0){
